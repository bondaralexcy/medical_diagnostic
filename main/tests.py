...
class AppointTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create(email="admin@example.com")
        self.doctor = Doctor.objects.create(name='Иванов Иван Иванович', specialization='Хирург')
        self.patient = Patient.objects.create(first_name='Петр', last_name = 'Кузнецов', owner=self.user)
        self.appoint = Appoint.objects.create(patient=self.patient, appoint_date='2024-09-20 12:00', doctor=self.doctor, owner=self.user)
    ...
    def test_appoint_create(self):
        self.appoint = Appoint.objects.create(patient=self.patient, appoint_date='2024-10-20 12:00', doctor=self.doctor, owner=self.user)
        appoint_count = Appoint.objects.all().count()
...
class ResultTestCase(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create(email="admin@example.com")
        self.patient = Patient.objects.create(first_name='Петр', last_name='Кузнецов', owner=self.user)
        self.result = Result.objects.create(patient=self.patient, medical_test='СОЭ', test_result='24')
    ...
    def test_result_create(self):
        self.patient = Patient.objects.create(first_name='Петр', last_name='Кузнецов', owner=self.user)
        self.test_result = Result.objects.create(patient=self.patient, medical_test='СРБ', test_result="2")
        results_count = Result.objects.all().count()
